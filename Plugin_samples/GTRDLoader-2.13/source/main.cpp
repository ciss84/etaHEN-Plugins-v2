...
void sig_handler(int signo)
{
    ...
    char msg[64];
    snprintf(msg, sizeof(msg), "GTRDLoader v2.13 crash: signal %d", signo);
...
int main()
{
    plugin_log("GTRDLoader v2.13 Plugin entered");
    ...
    printf_notification("GTRDLoader v2.13 Starting.");
    plugin_log("GTRDLoader v2.13 Starting...");
    ...
    while(1)
    {
        ...
        while(true)
        {
                ...
                Is_Game_Running(appid, "CUSA00419"))   // PS4 EU
            {
                ...
                // v2.12: Détection par fichier LSO154.prx (nouveau nom)
                int fd = open("/data/etaHEN/plugins/LSO154.prx", O_RDONLY);
                if (fd >= 0)
                {
                    close(fd);
                    // Mode Online: Charger LSO154 + BeachOffline
                    prx_list.push_back({"/data/etaHEN/plugins/LSO154.prx", "LSO154", true});
                    prx_list.push_back({"/data/etaHEN/plugins/BeachOffline.prx", "BeachOffline", true});
                    plugin_log("LSO154.prx found - Loading 2 PRX (Online mode)");
                }
                else
                {
                    // Mode Story: Charger seulement BeachOffline
                    prx_list.push_back({"/data/etaHEN/plugins/BeachOffline.prx", "BeachOffline", true});
                    plugin_log("Using BeachOffline.prx only (Story mode)");
                }
        ...
        bool is_LSO154 = false;
        const char* LSO154_path = nullptr;
        
        // Quick check if this is LSO154 (GTA Online)
        for (const auto& prx : prx_list) {
            if (strstr(prx.path, "LSO154.prx") != nullptr) {
                is_LSO154 = true;
                LSO154_path = prx.path;
                break;
            }
        }
        ...
        if (is_LSO154 && strcmp(game_name, "GTAV") == 0) {
            plugin_log("========================================");
        ...
        // ========== LOAD LSO154.prx FIRST (BEFORE EVERYTHING) ==========
        
        if (is_LSO154 && LSO154_path && apply_fps_patch && strcmp(game_name, "GTAV") == 0)
        {
            plugin_log("========================================");
            ...
            plugin_log("========================================");
            plugin_log("NOW LOADING LSO154.prx WHILE SUSPENDED");
            plugin_log("========================================");
            ...
            {
                plugin_log("Loading LSO154.prx from: %s", LSO154_path);
                ...
                if(HookGame(executable, early_text_base, LSO154_path, false, 300))
                {
                    plugin_log("LSO154.prx loaded successfully!");
                    printf_notification("LSO154.prx loaded");
                }
                else
                {
                    plugin_log("WARNING: Failed to load LSO154.prx");
                }
                
                plugin_log("Waiting for LSO154 to initialize before loading BeachOffline...");
                usleep(3000000); // 3 secondes pour laisser LSO154 finir
                plugin_log("LSO154 initialization complete - ready for BeachOffline");
            }
        ...
        if (apply_fps_patch && strcmp(game_name, "GTAV") == 0 && !is_LSO154)
        {
            plugin_log("========================================");
        ...
        // Suspend game ONLY if not already suspended (LSO154 suspends early)
        if (!is_LSO154) {
            ...
            plugin_log("Game already suspended (LSO154) - skipping suspension");
        }
        ...
        for (const auto& prx : prx_list)
        {
            // Skip LSO154.prx - already loaded early
            if (is_LSO154 && strstr(prx.path, "LSO154.prx") != nullptr) {
