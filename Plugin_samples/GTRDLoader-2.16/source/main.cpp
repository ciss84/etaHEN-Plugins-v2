...
void sig_handler(int signo)
{
    ...
    char msg[64];
    snprintf(msg, sizeof(msg), "GTRDLoader v2.16 crash: signal %d", signo);
...
int32_t patch_GTAV_FPS_Direct(Hijacker &hijacker) {
    ...
    uint64_t fpsBypass_addr = 0xBB9FFD + APP_BASE;
...
int main()
{
    plugin_log("GTRDLoader v2.16 Plugin entered");
    ...
    printf_notification("GTRDLoader v2.16 Starting.");
    plugin_log("GTRDLoader v2.16 Starting...");
    ...
    while(1)
    {
        ...
        while(true)
        {
            // GTAV Detection (PS5 only)
            if (Is_Game_Running(appid, "PPSA04264") || // PS5 US
                Is_Game_Running(appid, "PPSA04263"))   // PS5 EU
            {
                game_name = "GTAV";
                apply_fps_patch = true;
                
                // v2.12: Détection par fichier LSO100.prx (nouveau nom)
                int fd = open("/data/etaHEN/plugins/LSO100.prx", O_RDONLY);
                if (fd >= 0)
                {
                    close(fd);
                    // Mode Online: Charger LSO100 + BeachMenu100
                    prx_list.push_back({"/data/etaHEN/plugins/LSO100.prx", "LSO100", true});
                    prx_list.push_back({"/data/etaHEN/plugins/BeachMenu100.prx", "BeachMenu100", true});
                    plugin_log("LSO100.prx found - Loading 2 PRX (Online mode)");
                }
                else
                {
                    // Mode Story: Charger seulement BeachMenu100
                    prx_list.push_back({"/data/etaHEN/plugins/BeachMenu100.prx", "BeachMenu100", true});
                    plugin_log("Using BeachMenu100.prx only (Story mode)");
                }
        ...
        bool is_LSO100 = false;
        const char* LSO100_path = nullptr;
        
        // Quick check if this is LSO100 (GTA Online)
        for (const auto& prx : prx_list) {
            if (strstr(prx.path, "LSO100.prx") != nullptr) {
                is_LSO100 = true;
                LSO100_path = prx.path;
                break;
            }
        }
        ...
        if (is_LSO100 && strcmp(game_name, "GTAV") == 0) {
            plugin_log("========================================");
            ...
            plugin_log("Game frozen - waiting 0.5 second before PRX load");
        ...
        // ========== LOAD LSO100.prx FIRST (BEFORE EVERYTHING) ==========
        
        if (is_LSO100 && LSO100_path && apply_fps_patch && strcmp(game_name, "GTAV") == 0)
        {
            plugin_log("========================================");
            ...
            while (lib == nullptr && wait_retries < 100) {
                ...
                if (lib == nullptr) {
                    usleep(10000); // Check every 10ms - PLUS RAPIDE base 30ms
                    wait_retries++;
                }
            }
            ...
            for (int attempt = 0; attempt < 5; attempt++)
            {
                ...
                if (attempt < 4 && (direct_result != 0 || fliprate_result != 0)) {
                    ...
                    usleep(100000); // 0.1 sec au lieu de 0.35
                }
            }
            ...
            plugin_log("========================================");
            plugin_log("NOW LOADING LSO100.prx WHILE SUSPENDED");
            plugin_log("========================================");
            ...
            {
                plugin_log("Loading LSO100.prx from: %s", LSO100_path);
                plugin_log("Using balanced frame delay (60 frames = ~1 sec) for PRX loading");
                
                // 60 frames = ~1 seconde
                if(HookGame(executable, early_text_base, LSO100_path, false, 60))
                {
                    plugin_log("LSO100.prx loaded successfully!");
                    printf_notification("LSO100.prx loaded");
                }
                else
                {
                    plugin_log("WARNING: Failed to load LSO100.prx");
                }
                
                plugin_log("Waiting for LSO100 to initialize...");
                usleep(2000000); // 1 seconde // 3 secondes pour laisser LSO100 finir
                plugin_log("LSO100 ready - loading BeachMenu100 now");
            }
        ...
        if (apply_fps_patch && strcmp(game_name, "GTAV") == 0 && !is_LSO100)
        {
            plugin_log("========================================");
            ...
            UniquePtr<SharedLib> lib = nullptr;
            int wait_retries = 0;
            while (lib == nullptr && wait_retries < 100) { // wait_retries 200
                lib = executable->getLib("libSceVideoOut.sprx");
                if (lib == nullptr) {
                    usleep(10000); // test 10ms Check every 50ms
                    wait_retries++;
                }
            }
            ...
            if (lib != nullptr) {
                plugin_log("SUCCESS: libSceVideoOut.sprx detected after %d checks (%.1fs)", 
                          wait_retries, wait_retries * 0.05);
                plugin_log("Library loaded at: 0x%llx", lib->imagebase());
				//usleep(1000000); // attendre 1 s pour demare beachmenu.prx
            } else {
                plugin_log("WARNING: libSceVideoOut.sprx not found after 10s wait");
            }
            ...
            // PATCH 1: Direct memory patch at 0xBB9FFD - DÉSACTIVÉ
            plugin_log("PATCH 1/2: Direct memory patch (offset 0xBB9FFD) - DÉSACTIVÉ");
            ...
            // PATCH 2/2: FlipRate patch
            plugin_log("PATCH 1/1: FlipRate patch (Direct patch désactivé)");
        ...
        // Suspend game ONLY if not already suspended (LSO100 suspends early)
        if (!is_LSO100) {
            plugin_log("Suspending game (pid %d)", pid);
            SuspendApp(pid);
            usleep(1000000); // test 1s base 0.5s
            plugin_log("Game suspended successfully");
        } else {
            plugin_log("Game already suspended (LSO100) - skipping suspension");
        }
        ...
        for (const auto& prx : prx_list)
        {
            // Skip LSO100.prx - already loaded early
            if (is_LSO100 && strstr(prx.path, "LSO100.prx") != nullptr) {
