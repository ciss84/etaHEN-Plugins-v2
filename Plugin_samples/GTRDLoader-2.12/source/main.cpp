...
void sig_handler(int signo)
{
    ...
    char msg[64];
    snprintf(msg, sizeof(msg), "GTRDLoader v2.12 crash: signal %d", signo);
...
int main()
{
    plugin_log("GTRDLoader v2.12 Plugin entered");
    ...
    printf_notification("GTRDLoader v2.12 Starting.");
    plugin_log("GTRDLoader v2.12 Starting...");
    ...
    while(1)
    {
        ...
        while(true)
        {
                ...
                Is_Game_Running(appid, "CUSA00419"))   // PS4 EU
            {
                ...
                // v2.12: Détection par fichier LSO153.prx (nouveau nom)
                int fd = open("/data/etaHEN/plugins/LSO153.prx", O_RDONLY);
                if (fd >= 0)
                {
                    close(fd);
                    // Mode Online: Charger LSO153 + BeachMenu153
                    prx_list.push_back({"/data/etaHEN/plugins/LSO153.prx", "LSO153", true});
                    prx_list.push_back({"/data/etaHEN/plugins/BeachMenu153.prx", "BeachMenu153", true});
                    plugin_log("LSO153.prx found - Loading 2 PRX (Online mode)");
                }
                else
                {
                    // Mode Story: Charger seulement BeachMenu153
                    prx_list.push_back({"/data/etaHEN/plugins/BeachMenu153.prx", "BeachMenu153", true});
                    plugin_log("Using BeachMenu153.prx only (Story mode)");
                }
        ...
        if (is_lso153 && lso153_path && apply_fps_patch && strcmp(game_name, "GTAV") == 0)
        {
            ...
            plugin_log("========================================");
            plugin_log("NOW LOADING LSO153.prx WHILE SUSPENDED");
            plugin_log("========================================");
            ...
            if (early_text_base != 0)
            {
                ...
                plugin_log("Waiting for LSO153 to initialize before loading BeachMenu153...");
                usleep(3000000); // 3 secondes pour laisser LSO153 finir
                plugin_log("LSO153 initialization complete - ready for BeachMenu153");
            }
        ...
        for (const auto& prx : prx_list)
        {
            // Skip LSO153.prx - already loaded early
            if (is_lso153 && strstr(prx.path, "LSO153.prx") != nullptr) {
