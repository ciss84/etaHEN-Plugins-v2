...
{
    printf_notification("GTRDLoader v2.07 crashed with signal %d", signo);
}
...
extern "C"{
    ...
    // ========== NOUVEAUX SYSCALLS POUR etaHEN 2.3b SUR FIRMWARE 9.60 ==========
    #define SYS_PROC_CMD 107
    #define PROC_READ_MEM   10
    #define PROC_WRITE_MEM  11
    #define PROC_PROTECT    13
...
int g_firmware_version = 0;  // Version du firmware détectée au démarrage

// ========== FONCTIONS COMPATIBLES etaHEN 2.3b (FIRMWARE 9.60) ==========

// Détecter la version du firmware
int get_firmware_version() {
    // Protection contre les crashes
    int version[4] = {0, 0, 0, 0};
    size_t len = sizeof(version);
    
    // Essayer de lire la version du SDK
    int result = sysctlbyname("kern.sdk_version", version, &len, NULL, 0);
    
    if (result == 0 && version[0] > 0) {
        // Version format: (V0100 a V1060) 0x04030000 pour 4.03, 0x05500000 pour 5.50, 0x09600000 pour 9.60, 0x10600000 pour 10.60
        return version[0];
    }
    
    // Fallback: retourner 0 (utilisera la méthode classique hijacker.write)
    return 0;
}

// Fonction pour écrire dans la mémoire d'un processus (compatible toutes versions)
int eta_proc_write(int pid, uint64_t addr, void* data, size_t size) {
    // Utiliser la variable globale détectée au démarrage (pas de static ici)
    // Si firmware 9.00+ (0x09000000), utiliser syscalls etaHEN 2.3b
    if (g_firmware_version >= 0x09000000) {
        plugin_log("[etaHEN 2.3b] Using FW 9.xx method for write");
        
        // Étape 1: Changer les protections mémoire (RWX)
        uint64_t aligned_addr = addr & ~0xFFFULL;
        uint64_t aligned_size = ((size + 0xFFF) & ~0xFFFULL);
        
        int prot_result = syscall(SYS_PROC_CMD, PROC_PROTECT, pid, aligned_addr, aligned_size, 7);
        
        if (prot_result != 0) {
            plugin_log("[etaHEN 2.3b] WARNING: mprotect returned %d", prot_result);
        }
        
        usleep(100000); // 100ms
        
        // Étape 2: Écrire les données
        int write_result = syscall(SYS_PROC_CMD, PROC_WRITE_MEM, pid, addr, data, size);
        
        if (write_result != 0) {
            plugin_log("[etaHEN 2.3b] ERROR: Write failed with code %d", write_result);
            return write_result;
        }
    ...
    // Sinon (1.00-8.60), utiliser la méthode classique (retourner -1 pour utiliser hijacker.write)
    plugin_log("[Classic] Using FW 1.00-8.60 method (hijacker.write)");
    return -1; // Signal pour utiliser hijacker.write à la place
}

// Fonction pour lire la mémoire d'un processus (compatible toutes versions)
int eta_proc_read(int pid, uint64_t addr, void* data, size_t size) {
    if (g_firmware_version >= 0x09000000) {
        return syscall(SYS_PROC_CMD, PROC_READ_MEM, pid, addr, data, size);
    }
    
    // Signal pour utiliser hijacker.read
    return -1;
}
...
int32_t patch_GTAV_FPS_Direct(Hijacker &hijacker, pid_t pid) {
    ...
    // Read original bytes
    uint8_t original[8];
    if (eta_proc_read(pid, absolute_addr, original, sizeof(original)) == -1) {
    ...
    // Essayer d'abord eta_proc_write (FW 9.xx)
    int write_result = eta_proc_write(pid, absolute_addr, patch, sizeof(patch));
    
    // Si retourne -1, utiliser hijacker.write (FW 5.50-8.00)
    if (write_result == -1) {
        plugin_log("[GTAV] Using hijacker.write (FW 5.50-8.00)");
        hijacker.write(absolute_addr, patch);
        write_result = 0;
    }
    
    if (write_result != 0) {
        plugin_log("[GTAV] ERROR: Write operation failed!");
    ...
    // Verify patch
    uint8_t verify[8];
    if (eta_proc_read(pid, absolute_addr, verify, sizeof(verify)) == -1) {
...
int32_t patch_SetFlipRate(Hijacker &hijacker, pid_t pid, const char* game_name) {
    ...
    uint8_t original[16];
    if (eta_proc_read(pid, fliprate_addr, original, sizeof(original)) == -1) {
    ...
    // Essayer d'abord eta_proc_write (FW 9.xx)
    int write_result = eta_proc_write(pid, fliprate_addr, patch, sizeof(patch));
    
    // Si retourne -1, utiliser hijacker.write (FW 5.50-8.00)
    if (write_result == -1) {
        plugin_log("[%s] Using hijacker.write (FW 5.50-8.00)", game_name);
        hijacker.write(fliprate_addr, patch);
        write_result = 0;
    }
    
    if (write_result != 0) {
        plugin_log("[%s] ERROR: Write operation failed!", game_name);
        plugin_log("========== FPS PATCH FAILED ==========");
        return -1;
    }
    ...
    uint8_t verify[16];
    if (eta_proc_read(pid, fliprate_addr, verify, sizeof(verify)) == -1) {
...
int main()
{
    plugin_log("GTRDLoader v2.07 Plugin entered");
    ...
    // ========== DÉTECTION DU FIRMWARE UNE SEULE FOIS ==========
    g_firmware_version = get_firmware_version();
    plugin_log("Firmware version detected: 0x%08X", g_firmware_version);
    
    if (g_firmware_version >= 0x09000000) {
        plugin_log("Using etaHEN 2.3b syscalls (FW 9.xx)");
    } else {
        plugin_log("Using classic hijacker.write (FW 5.50-8.00)");
    }
    ...
    printf_notification("GTRDLoader v2.07 Starting.");
    plugin_log("GTRDLoader v2.07 Starting...");
    ...
    while(1)
    {
        ...
        if (is_lso153 && strcmp(game_name, "GTAV") == 0) {
            plugin_log("========================================");
            plugin_log("GTA V ONLINE DETECTED - EARLY SUSPEND");
            plugin_log("========================================");
            plugin_log("Suspending IMMEDIATELY (pid %d)", pid);
        ...
        size_t text_size = executable->getEboot()->getTextSection()->sectionLength();
        
        plugin_log("Executable base (ASLR): 0x%llx", executable->getEboot()->imagebase());
        plugin_log("Text section start: 0x%llx", text_base);
        plugin_log("Text section size: 0x%zX (%zu bytes)", text_size, text_size);
        ...
        // ========== GTA V ONLINE (LSO153): FPS PATCHES AVANT CHARGEMENT PRX ==========
        if (is_lso153 && apply_fps_patch && strcmp(game_name, "GTAV") == 0)
        {
            plugin_log("========================================");
            ...
            if (lib != nullptr) {
                plugin_log("SUCCESS: libSceVideoOut.sprx detected after %d checks (%.1fs)", 
                          wait_retries, wait_retries * 0.03);
                plugin_log("Library loaded at: 0x%llx", lib->imagebase());
            } else {
                plugin_log("WARNING: libSceVideoOut.sprx not found after 3s wait");
            }
            ...
            for (int attempt = 0; attempt < 5; attempt++)
            {
                ...
                if (direct_result != 0) {
                    ...
                    direct_result = patch_GTAV_FPS_Direct(*executable, pid);
                    if (direct_result == 0) {
                ...
                if (fliprate_result != 0) {
                    ...
                    fliprate_result = patch_SetFlipRate(*executable, pid, game_name);
                    if (fliprate_result == 0) {
                ...
                if (attempt < 4 && (direct_result != 0 || fliprate_result != 0)) {
                    plugin_log("Waiting 0.35s before next attempt...");
                    usleep(350000);
                }
            }
        ...
        // ========== GTA V OFFLINE: FPS PATCHES AVANT LA SUSPENSION ==========
        if (apply_fps_patch && strcmp(game_name, "GTAV") == 0 && !is_lso153)
        {
            ...
            // Wait for libSceVideoOut.sprx to be loaded
            UniquePtr<SharedLib> lib = nullptr;
            int wait_retries = 0;
            int max_wait = 200; // 10 seconds max (200 * 50ms)
            
            while (lib == nullptr && wait_retries < max_wait) {
                lib = executable->getLib("libSceVideoOut.sprx");
                if (lib == nullptr) {
            ...
            int direct_result = patch_GTAV_FPS_Direct(*executable, pid);
            ...
            int patch_result = patch_SetFlipRate(*executable, pid, game_name);
        ...
        for (const auto& prx : prx_list)
        {
            ...
            // LSO153 nécessite un délai plus long (5 sec) pour l'anti-cheat
            if (is_lso153 && strstr(prx.path, "LSO153bea.prx") != nullptr) {
                frame_delay = 300; // 5 secondes pour GTA Online
            ...
            for (int attempt = 0; attempt < 2; attempt++)
            {
                ...
                if(HookGame(executable, text_base, prx.path, false, frame_delay))
                {
                    ...
                    // Notification spéciale pour LSO153bea
                    if (is_lso153 && strstr(prx.path, "LSO153bea.prx") != nullptr) {
