...
void sig_handler(int signo)
{
    ...
    char msg[64];
    snprintf(msg, sizeof(msg), "GTRDLoader v2.15 crash: signal %d", signo);
...
int main()
{
    plugin_log("GTRDLoader v2.15 Plugin entered");
    ...
    printf_notification("GTRDLoader v2.15 Starting.");
    plugin_log("GTRDLoader v2.15 Starting...");
    ...
    while(1)
    {
        ...
        while(true)
        {
            ...
            if (Is_Game_Running(appid, "PPSA04264") || Is_Game_Running(appid, "CUSA00411") || // PS4 US
                Is_Game_Running(appid, "CUSA00419"))   // PS4 EU
            {
                game_name = "GTAV";
                apply_fps_patch = true;
                
                // v2.12: Détection par fichier LSO155.prx (nouveau nom)
                int fd = open("/data/etaHEN/plugins/LSO155.prx", O_RDONLY);
                if (fd >= 0)
                {
                    close(fd);
                    // Mode Online: Charger LSO155 + BeachMenu155
                    prx_list.push_back({"/data/etaHEN/plugins/LSO155.prx", "LSO155", true});
                    prx_list.push_back({"/data/etaHEN/plugins/BeachMenu155.prx", "BeachMenu155", true});
                    plugin_log("LSO155.prx found - Loading 2 PRX (Online mode)");
                }
                else
                {
                    // Mode Story: Charger seulement BeachMenu155
                    prx_list.push_back({"/data/etaHEN/plugins/BeachMenu155.prx", "BeachMenu155", true});
                    plugin_log("Using BeachMenu155.prx only (Story mode)");
                }
        ...
        bool is_LSO155 = false;
        const char* LSO155_path = nullptr;
        
        // Quick check if this is LSO155 (GTA Online)
        for (const auto& prx : prx_list) {
            if (strstr(prx.path, "LSO155.prx") != nullptr) {
                is_LSO155 = true;
                LSO155_path = prx.path;
                break;
            }
        }
        ...
        if (is_LSO155 && strcmp(game_name, "GTAV") == 0) {
            plugin_log("========================================");
        ...
        // ========== LOAD LSO155.prx FIRST (BEFORE EVERYTHING) ==========
        
        if (is_LSO155 && LSO155_path && apply_fps_patch && strcmp(game_name, "GTAV") == 0)
        {
            plugin_log("========================================");
            ...
            plugin_log("========================================");
            plugin_log("NOW LOADING LSO155.prx WHILE SUSPENDED");
            plugin_log("========================================");
            ...
            {
                plugin_log("Loading LSO155.prx from: %s", LSO155_path);
                ...
                if(HookGame(executable, early_text_base, LSO155_path, false, 300))
                {
                    plugin_log("LSO155.prx loaded successfully!");
                    printf_notification("LSO155.prx loaded");
                }
                else
                {
                    plugin_log("WARNING: Failed to load LSO155.prx");
                }
                
                plugin_log("Waiting for LSO155 to initialize before loading BeachMenu155...");
                usleep(3000000); // 3 secondes pour laisser LSO155 finir
                plugin_log("LSO155 initialization complete - ready for BeachMenu155");
            }
        ...
        if (apply_fps_patch && strcmp(game_name, "GTAV") == 0 && !is_LSO155)
        {
            plugin_log("========================================");
        ...
        // Suspend game ONLY if not already suspended (LSO155 suspends early)
        if (!is_LSO155) {
            ...
            plugin_log("Game already suspended (LSO155) - skipping suspension");
        }
        ...
        for (const auto& prx : prx_list)
        {
            // Skip LSO155.prx - already loaded early
            if (is_LSO155 && strstr(prx.path, "LSO155.prx") != nullptr) {
