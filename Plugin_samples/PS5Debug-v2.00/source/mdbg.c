...
#define MAX_MDBG_LENGTH (1024 * 1024 * 16)  // 16MB max for safety
...
void userland_copyin(int pid, const void *src, uintptr_t dst, size_t length) {
	// Validate parameters
	if (pid <= 0) {
		printf("[mdbg] Invalid PID: %d\n", pid);
		return;
	}
	if (!src) {
		printf("[mdbg] Source pointer is NULL\n");
		return;
	}
	if (length == 0) {
		return;  // Nothing to do
	}
	if (length > MAX_MDBG_LENGTH) {
		printf("[mdbg] Length %zu exceeds maximum %d\n", length, MAX_MDBG_LENGTH);
		return;
	}
	if (dst == 0) {
		printf("[mdbg] Destination address is NULL\n");
		return;
	}
...
void userland_copyout(int pid, uintptr_t src, void *dst, size_t length) {
	...
	if (!dst) {
		printf("[mdbg] Destination pointer is NULL\n");
		return;
	}
	if (length == 0) {
		return;  // Nothing to do
	...
	if (src == 0) {
		printf("[mdbg] Source address is NULL\n");
		return;
	}
