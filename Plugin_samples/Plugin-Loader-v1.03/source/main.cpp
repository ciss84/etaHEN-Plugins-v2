...
void sig_handler(int signo)
{
    ...
    char msg[64];
    snprintf(msg, sizeof(msg), "GTRDLoader v2.19 crash test: signal %d", signo);
...
int main()
{
    plugin_log("GTRDLoader v2.19 Plugin entered");
    ...
    printf_notification("GTRDLoader v2.19 Starting.");
    plugin_log("GTRDLoader v2.19 Starting...");
    ...
    while(1)
    {
        ...
        while(true)
        {
          ...
          if (Is_Game_Running(appid, "PPSA04264") || Is_Game_Running(appid, "PPSA04263"))
          {
             ...
             // Vérifier si /data/Ps5-lso-online existe pour détecter le mode
             int fd = open("/data/Ps5-lso-online", O_RDONLY);
             if (fd >= 0)
             {
               close(fd);
               // Mode Online: Charger LSO100 + BeachMenu100
                ...
                plugin_log("/data/Ps5-lso-online found - Loading LSO100 + BeachMenu100 (Online mode)");
             }
             else
             {
                // Mode Story: Charger seulement BeachMenu100
                prx_list.push_back({"/data/etaHEN/plugins/BeachMenu100.prx", "BeachMenu100", true});
                plugin_log("/data/Ps5-lso-online not found - Using BeachMenu100 only (Story mode)");
             }
        ...
        if (is_LSO100 && LSO100_path && apply_fps_patch && strcmp(game_name, "GTAV") == 0)
        {
            ...
            if (early_text_base != 0)
            {
                ...
                plugin_log("Waiting for LSO100 to initialize before loading BeachMenu100...");
                usleep(750000); // 0.75 secondes pour laisser LSO100 finir
                plugin_log("LSO100 initialization complete - ready for BeachMenu100");
            }
